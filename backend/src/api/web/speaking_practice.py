...
Runtime retrieval is GitHub-backed. The old Drive-backed creation endpoint is
kept as a disabled legacy route until final route deletion.
"""
...
def speaking_practice_create(user_id: str):
    """POST /web/speaking-practice/sets - disabled legacy Drive flow."""
    del user_id
    return (
        ResponseBuilder()
        .error(
            error={"code": "legacy_drive_flow_disabled"},
            message="Legacy Drive-backed speaking practice creation is disabled.",
            status_code=410,
        )
        .build()
    )
