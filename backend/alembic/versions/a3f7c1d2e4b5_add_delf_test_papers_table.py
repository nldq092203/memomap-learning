...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "exercise_count", sa.Integer(), nullable=False, server_default=sa.text("0")
        ),
    ...
    # Create indexes
    op.create_index(
        "ix_delf_test_papers_level_section_status",
        "delf_test_papers",
...
def downgrade() -> None:
    """Downgrade schema."""
    op.drop_index(
        "ix_delf_test_papers_level_section_status", table_name="delf_test_papers"
    )
