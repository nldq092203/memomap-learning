        ...
        <ScrollArea className="h-[min(62vh,520px)] rounded-[18px] border border-slate-200 bg-white">
          <div className="mx-auto max-w-4xl space-y-4 p-5 sm:p-7">
            {textBlocks.map((block, i) => (
              block.kind === "heading" ? (
                <h4 key={`${block.text}-${i}`} className="text-base font-bold uppercase leading-7 tracking-wide text-slate-950 sm:text-lg">
                  {block.text}
                </h4>
              ) : (
                <p key={`${block.text}-${i}`} className="text-[15px] leading-8 text-slate-800 sm:text-base">
                  {block.text}
                </p>
              )
            ))}
