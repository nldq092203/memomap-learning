  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      ...
      {loading ? (
        ...
        <ScrollArea className="pr-2 sm:pr-4 md:h-[640px]">
          <div className="grid gap-3 sm:grid-cols-2">
            {tests.map((test, index) => (
              <button
                ...
                className="group min-h-[128px] rounded-[22px] border border-[var(--vintage-soft-sandstone)] bg-[var(--vintage-feather-white)]/92 p-4 text-left shadow-[0_10px_26px_rgba(74,51,35,0.06)] transition-all hover:-translate-y-0.5 hover:border-[var(--vintage-desert-rock)] hover:shadow-[0_14px_34px_rgba(74,51,35,0.12)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--vintage-desert-rock)]"
              >
                <div className="flex h-full min-w-0 flex-col justify-between gap-5">
                  <div className="space-y-3">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="inline-flex items-center gap-2 rounded-full bg-[var(--vintage-porcelain-mist)] px-2.5 py-1 text-[11px] font-semibold text-[var(--vintage-muted-ink)]">
                        <Icon className="h-3.5 w-3.5" />
                        {sectionName}
                      </span>
                      <span className="rounded-full bg-[var(--vintage-cream)] px-2.5 py-1 text-[11px] font-semibold text-[var(--vintage-desert-rock)]">
                        {index + 1}
                      </span>
                      {test.status === "draft" && (
                    ...
                    <h2 className="text-xl font-semibold tracking-tight text-[var(--vintage-ink)]">
                      {formatExerciseTitle(test.test_id)}
                    </h2>
                  </div>

                  <div className="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex min-w-0 items-center gap-2 text-sm font-medium text-[var(--vintage-muted-ink)]">
                      <Clock3 className="h-4 w-4" />
                      <span>{test.exercise_count} question{test.exercise_count > 1 ? "s" : ""}</span>
                    </div>
                    <div className="inline-flex h-9 items-center justify-between gap-3 rounded-full bg-[var(--vintage-desert-rock)] px-4 text-sm font-semibold text-white transition-colors group-hover:bg-[#8f7763] sm:min-w-[132px]">
                      Ouvrir
